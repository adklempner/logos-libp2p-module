...
// on the Qt thread — rlnEnable warms it up so the libp2p-thread fetcher only
// ever reuses the existing instance.
...
int Libp2pModuleImpl::rlnFetcherTrampoline(const char* methodName, const char* params,
    ...
    LogosAPI* api = self->m_logosAPI;
    if (!api) return fail("rln fetcher: LogosAPI not ready (rlnEnable not called)");
